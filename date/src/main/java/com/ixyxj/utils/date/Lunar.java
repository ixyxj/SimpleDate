...
public final class Lunar {
    ...
    public Lunar(Calendar cal) {
        int leapMonth;
        Date baseDate = DateUtil.parseToDate("1900年1月31日");
        // 求出和1900年1月31日相差的天数
        ...
        for (iYear = 1900; iYear < 2050 && offset > 0; iYear++) {
            daysOfYear = LunarUtil.getYearDays(iYear);
            offset -= daysOfYear;
            monCyl += 12;
        }
        if (offset < 0) {
        ...
        leapMonth = LunarUtil.getLeapMonth(iYear); // 闰哪个月,1-12
        leap = false;
        ...
        for (iMonth = 1; iMonth < 13 && offset > 0; iMonth++) {
            ...
            if (leapMonth > 0 && iMonth == (leapMonth + 1) && !leap) {
                --iMonth;
                leap = true;
                daysOfMonth = LunarUtil.getLeapDays(year);
            } else
                daysOfMonth = LunarUtil.getMonthDays(year, iMonth);
            offset -= daysOfMonth;
            // 解除闰月
